...
def main():
    ...
    try:
        ...
        # On macOS, Python installations often lack root certificates, causing CERTIFICATE_VERIFY_FAILED.
        # We retry with an unverified context if that happens.
        if hasattr(e, "reason") and "CERTIFICATE_VERIFY_FAILED" in str(e.reason):
            try:
                import ssl
                context = ssl._create_unverified_context()
                with urllib.request.urlopen(req, timeout=10, context=context) as resp:
                    print(f"[ai-log] Submitted {len(entries)} entries (SSL bypassed) → {resp.status}", file=sys.stderr)
            except Exception as retry_err:
                _restore_pending(pending)
                print(f"[ai-log] Submit failed after SSL bypass: {retry_err} — logs kept locally.", file=sys.stderr)
                sys.exit(0)
        else:
